...
import styles from './TopTrack.scss';
...
export default class TopTrack extends Component {
  static propTypes = {
    ...
    return (
      <li className={styles.track}>
        ...
        <span className={cx({
          ...
          <button className={styles.track__vote_minus} onClick={() => this.vote(-1) }>-</button>
          <var className={styles.track__votes_rating}>{r}</var>
          <button className={styles.track__vote_plus} onClick={() => this.vote(1) }>+</button>
        </span>
        <span className={styles.track__progress}>
          <div className={styles.track__progress_bar} style={{ width: `30%` }} />
        </span>
      </li>
    );
  }
